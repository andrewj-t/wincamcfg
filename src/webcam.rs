...
pub fn parse_property_value(property_name: &str, value_str: &str) -> Result<(i32, bool)> {
    ...
    if !value_str.chars().all(|c| c.is_alphanumeric() || c == '-' || c == ' ') {
...
fn get_supported_values(property_name: &str, min: i32, max: i32, caps: i32) -> String {
    let base_values = match property_name {
        "PowerlineFrequency" => {
            let mut values = vec![];
            if min <= 0 && max >= 0 { values.push("Disabled"); }
            if min <= 1 && max >= 1 { values.push("50Hz"); }
            if min <= 2 && max >= 2 { values.push("60Hz"); }
            if min <= 3 && max >= 3 { values.push("Auto"); }
            values.join(", ")
        },
        "ColorEnable" | "BacklightCompensation" => {
            let mut values = vec![];
            if min <= 0 && max >= 0 { values.push("Off"); }
            if min <= 1 && max >= 1 { values.push("On"); }
            values.join(", ")
        },
        _ => format!("{}-{}", min, max),
    };
...
pub fn enumerate_devices() -> Result<Vec<DeviceInfo>> {
    unsafe {
        ...
        let hr = dev_enum.CreateClassEnumerator(&CLSID_VIDEO_INPUT_DEVICE_CATEGORY, &mut enum_moniker, 0);
        ...
        loop {
            ...
            if let Some(mon) = &monikers[0] {
                ...
                if let Ok(props) = get_video_proc_amp_properties(mon) {
                    debug!(property_count = props.len(), "VideoProcAmp properties enumerated");
                ...
                if let Ok(props) = get_camera_control_properties(mon) {
                    debug!(property_count = props.len(), "CameraControl properties enumerated");
...
unsafe fn get_property_string(moniker: &IMoniker, prop_name: &str) -> Result<String> {
    ...
    unsafe {
        let prop_bag: IPropertyBag = moniker.BindToStorage(None, None)
        ...
        prop_bag.Read(PCWSTR(prop_name_wide.as_ptr()), var.as_mut_ptr(), None)
...
unsafe fn get_driver_info_from_path(device_path: &str) -> Result<DriverInfo> {
    let device_instance_id = parse_device_instance_id(device_path)
        .with_context(|| format!("Failed to parse device instance ID from path: {}", device_path))?;
    
    unsafe {
        let device_id_wide: Vec<u16> = device_instance_id.encode_utf16().chain(std::iter::once(0)).collect();
        ...
        let (driver_version, driver_date, driver_path) = get_driver_info_from_cm(devinst).ok().unwrap_or((None, None, None));
...
fn expand_indirect_string(s: &str) -> Option<String> {
    ...
    unsafe {
        ...
        let hr = SHLoadIndirectString(
            PCWSTR(s_wide.as_ptr()),
            &mut buffer,
            None,
        );
...
unsafe fn get_driver_info_from_cm(devinst: u32) -> Result<(Option<String>, Option<String>, Option<String>)> {
    unsafe {
        use windows::Win32::System::Registry::*;
...
unsafe fn read_reg_value(hkey: &windows::Win32::System::Registry::HKEY, value_name: &str) -> Result<String> {
    unsafe {
        use windows::Win32::System::Registry::*;
...
unsafe fn get_video_proc_amp_properties(moniker: &IMoniker) -> Result<Vec<PropertyInfo>> {
    ...
    unsafe {
        ...
        let video_proc_amp: IAMVideoProcAmp = filter.cast()
        ...
        trace!(property_count = properties.len(), "Enumerating VideoProcAmp properties");
        for property in properties {
            ...
            if video_proc_amp.GetRange(prop_id, &mut min, &mut max, &mut step, &mut default, &mut caps).is_ok() {
                trace!(property = %name, min, max, step, default, caps, "GetRange successful");
                let mut value = 0;
                let mut flags_val = 0;
                let current_formatted = if video_proc_amp.Get(prop_id, &mut value, &mut flags_val).is_ok() {
    ...
    debug!(property_count = capabilities.len(), "VideoProcAmp property enumeration complete");
    Ok(capabilities)
    }
}
...
unsafe fn get_camera_control_properties(moniker: &IMoniker) -> Result<Vec<PropertyInfo>> {
    ...
    unsafe {
        ...
        let camera_control: IAMCameraControl = filter.cast()
        ...
        trace!(property_count = properties.len(), "Enumerating CameraControl properties");
        for property in properties {
            let prop_id: i32 = property.into();
            let name = property.to_string();
            let mut min = 0;
            ...
            if camera_control.GetRange(prop_id, &mut min, &mut max, &mut step, &mut default, &mut caps).is_ok() {
                trace!(property = %name, min, max, step, default, caps, "GetRange successful");
                let mut value = 0;
                let mut flags_val = 0;
                let current_formatted = if camera_control.Get(prop_id, &mut value, &mut flags_val).is_ok() {
                    trace!(property = %name, value, flags = flags_val, "Get successful");
                    // If auto mode is enabled, return "Auto" as the current value
                    if (caps & CAPABILITY_AUTO != 0) && (flags_val & CAPABILITY_AUTO != 0) {
                        Some("Auto".to_string())
    ...
    debug!(property_count = capabilities.len(), "CameraControl property enumeration complete");
    Ok(capabilities)
    }
}
...
pub fn set_video_proc_amp_property(device: &DeviceInfo, property: VideoProcAmpProperty, value: i32, auto: bool) -> Result<()> {
    unsafe {
        let _com = ComGuard::new()?;
        
        let target_path = device.device_path.as_ref()
            .context("Device path not available")?;
        
        let mon = find_device_by_path(target_path)?;
        let filter: IBaseFilter = mon.BindToObject(None, None)
            .context("Failed to bind to device filter")?;
        let video_proc_amp: IAMVideoProcAmp = filter.cast()
            .context("Failed to get VideoProcAmp interface")?;
        
        let flags = if auto { CAPABILITY_AUTO } else { CAPABILITY_MANUAL };
        video_proc_amp.Set(property.into(), value, flags)
            .with_context(|| format!("Failed to set VideoProcAmp property {} to value {}", property, value))?;
...
pub fn set_camera_control_property(device: &DeviceInfo, property: CameraControlProperty, value: i32, auto: bool) -> Result<()> {
    unsafe {
        let _com = ComGuard::new()?;
        ...
        let flags = if auto { CAPABILITY_AUTO } else { CAPABILITY_MANUAL };
        camera_control.Set(property.into(), value, flags)
            .with_context(|| format!("Failed to set CameraControl property {} to value {}", property, value))?;
...
pub fn set_property(device: &DeviceInfo, property_name: &str, value_str: &str) -> Result<()> {
    ...
    let property_info = device.video_proc_amp_properties.iter()
        .find(|p| p.name.eq_ignore_ascii_case(property_name))
        .map(|p| (p, PropertyType::VideoProcAmp))
        .or_else(|| device.camera_control_properties.iter()
            .find(|p| p.name.eq_ignore_ascii_case(property_name))
            .map(|p| (p, PropertyType::CameraControl)));
    
    let (prop_info, property_type) = property_info
        .ok_or_else(|| anyhow::anyhow!("Property '{}' not found on device '{}'", 
            property_name, device.name.as_deref().unwrap_or("Unknown")))?;
    ...
    if !auto_mode {
            ...
            && (numeric_value < min || numeric_value > max)
        {
            anyhow::bail!(
                ...
                numeric_value, property_name, prop_info.supported_values, prop_info.supported_values
            );
        }
    }
