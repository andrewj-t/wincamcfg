...
- **Write verification** — after the writes for a device, `Device::read_back` re-binds the filter (a fresh handle) and `Written::persisted_in` compares. Some cameras keep a value only while a handle is open (C920 PowerlineFrequency); such writes are reported as failures. Reads through the *same* handle would not detect this, so do not "optimise" the re-bind away.
